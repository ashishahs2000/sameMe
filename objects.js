var Guy = new Class({
	initialize: function(gX,gY){
		...
		this.lightningStrikeChargeMin = 25; //Minimum weapon charge (fireing fails when current charge below this)
		this.lightningStrikeChargeMax = 300; //Maximum weapon charge (charging stops at this point)
		...
		this.moveBlurLength = 1; //How many xSpeeds and ySpeeds the blur covers
		
		
		/*
		*	Prepare stats. Woo boy, this bit sure is complicated. Roll eyes.
		*/
		...
		this.stats['wasteHealthy'] = 0; //Wasted healthy by bumping into him.
		this.stats['salvageHealthy'] = 0; //Got some use from healthy by shocking him after wasting.
		
		this.stats['bulletsSpawned'] = 0; //Excludes orbital lasers, depth charges (primary stage), and healthy bullets (maybe?).
		this.stats["depthChargesDropped"] = 0;
		this.stats["depthChargesDisarmed"] = 0;
		this.stats["damagePossible"] = 0; //Total damage possible, excluding orbital lasers. (Which would be a ton)
		...
		//Unused
		this.stats['attacks'] = 0; //How many times you used your attack
		this.stats['energyExpended'] = 0; // How much energy used to make those attacks
		this.stats['failedAttacks'] = 0; //Number of times you tried to attack but didn't due to lack of energy.
		
		this.stats['healed'] = 0; //How much you've been healed, excluding max health increases.
		this.stats['overHealed'] = 0; //How much you've been healed beyond your max health.
	...
	quake: function(){
		/*On laser strike, the ground shakes. Unused because I like keeping my lunch.*/
		//this.quakeTime = 30;
	},
	handleQuake: function(){
		if(this.quakeTime != 0){
			if(this.quakeTime == 1){
				...
				$('playField').style.top = Math.round(2 * this.quakeTime * Math.random() - this.quakeTime) + "px";
				$('playField').style.left= Math.round(2 * this.quakeTime * Math.random() - this.quakeTime) + "px";
				this.quakeTime--;
			}
		}
	},
	...
	dealDamage: function(amount,type,forceX,forceY,forceScale){
		if(!this.isDead){
			...
			if(this.health <= 0){
				...
				//Satellite
				if(type == "orbital"){type = "n orbital laser"}
				//Tank
				if(type == "flak"){type = " flak cannon"}
				if(type == "artillery"){type = "n artillery cannon"}
				//Plane
				if(type == "plane"){type = " plane gun"}
				if(type == "cows"){type = " plane crash"}
				if(type == "depth"){type = " depth charge"}
	...
	drawSelf: function(){
		...
		for(var i = this.moveBlurAmount; i > 0; i--){
			if(this.lightningImpact>0){
				...
				//ordinance.push(new DownwardAirShock(this.xPos-i*(this.xSpeed/this.moveBlurAmount),this.yPos-i*(this.ySpeed/this.moveBlurAmount),this.lightningImpact/10+((1/this.moveBlurAmount)*(this.moveBlurAmount-i))*2,0));
				//Power Diamonds				ordinance.push(new DownwardAirShock(this.xPos-i*(this.xSpeed/this.moveBlurAmount),this.yPos-i*(this.ySpeed/this.moveBlurAmount),this.lightningImpact/120+(i/this.moveBlurAmount)*(this.moveBlurAmount-i),0));
				//ordinance.push(new DownwardAirShock(this.xPos-i*(this.xSpeed/this.moveBlurAmount),this.yPos-i*(this.ySpeed/this.moveBlurAmount),(1-i/50)+this.lightningImpact/120,this.moveBlurAmount-i));
				ordinance.push(new DownwardAirShock(this.xPos-i*(this.xSpeed/this.moveBlurAmount),this.yPos-i*(this.ySpeed/this.moveBlurAmount),(this.lightningImpact-this.lightningStrikeChargeMin)/6+12, ((.15)/(this.moveBlurAmount))*i));
				if(true){
					context.restore();
			...
			*/
			...
			context.fillStyle = "rgba(255,255,255,"+temp+")";
			context.fillRect(-3-i*(this.moveBlurLength*this.xSpeed/this.moveBlurAmount),-12+i*(this.moveBlurLength*this.ySpeed/this.moveBlurAmount),6,12);
		}
...
//			this.xPos *= -1;
	...
	goHorizontal: function(amount){if(this.goingHorizontal == 0 || amount == 0){this.goingHorizontal = amount;}},
	goVertical: function(amount){if(this.goingVertical == 0 || amount == 0){this.goingVertical = amount;}},
	...
	lightningShield: function(){return;}
});
var DownwardAirShock = new Class({
	initialize: function(x,y,spread,opacityOffset){
		this.xPos = x;
		this.yPos = y;
		this.spread = this.originalSpread = spread; //How far in either direction the shockwave goes.
	...
	DESTROYDESTROYDESTROY: function(){
		if(this.isVolatile){
		//	if(enemies[i].distanceFrom(this.xPos) < (this.spread+10) && Math.abs(enemies[i].yPos - this.yPos) < (this.spread+5) ){
				
			if(healthy.isCloseTo(this.xPos,this.yPos,this.spread+12,14)){
				healthy.triggerHealthGive();
			}
			for(var i = 0; i < enemies.length; i++){
				if(!enemies[i].isInvulnerable && enemies[i].distanceFrom(this.xPos) < (this.spread+10) && Math.abs(enemies[i].yPos - this.yPos) < (this.spread+5) ){
...
var GroundShock = new Class({
	initialize: function(x,y,spread){
		this.xPos = x;
		this.yPos = y;
		this.spread = spread*1+50; //How far in either direction the shockwave goes.
	...
	DESTROYDESTROYDESTROY: function(){
		if(this.isVolatile){
			for(var i = 0; i < enemies.length; i++){
				if(!enemies[i].isInvulnerable && enemies[i].yPos <= 0 && enemies[i].distanceFrom(this.xPos) < this.ringSize){
					//enemies[i].shouldExist = false;
					enemies[i].die();
				}
			}
	...
	drawSelf: function(){
		...
		if(this.ringSize > 0){
			...
			for(var i = 0; i < this.rings.length; i++){
				context.strokeStyle = "rgba(255,255,255,"+((i)/(this.maxRings) - (this.ringOffset)/(this.maxRings))+")";
				context.beginPath();		
				context.arc(this.xPos,fHeight-this.yPos,(this.rings[i]),(350)*(Math.PI/180),360*(Math.PI/180),false);
				context.stroke();
		
				context.beginPath();		
				context.arc(this.xPos,fHeight-this.yPos,(this.rings[i]),180*(Math.PI/180),(190)*(Math.PI/180),false);
				context.stroke();
			}
		}
...
var Tank = new Class({
	initialize: function(tier,fromSide){
		...
		if(tier >= 6){
			random = Math.random();
			if(tier == 6 && random < .2){this.usesPEW = true;}
			if(tier == 7 && random < .4){this.usesPEW = true;}
			if(tier >= 8 && random < .6){this.usesPEW = true;}
		}
		...
		if(this.reloadCount <= 0){
			if(this.salvoCount == 0){
				...
				while(this.salvoCount != 0){
					...
					//Bullet.initialize: function(x,y,heading,speed,scale,damage,seaks,gravityMultiplier,inAccuracy){
					if(this.usesDrainy){
						//this.salvoCount = 0;
						ordinance.push(new Bullet(this.xPos,this.yPos,angleTemp,8*Math.random()+25,3,750,0,0,this.salvoCount/4,"drain"));
					} else if(this.usesPEW){
						ordinance.push(new Bullet(this.xPos,this.yPos,angleTemp,8*Math.random()+25,.1,1,0,0,this.salvoCount/4,"flak"));
					} else {
						ordinance.push(new Bullet(this.xPos,this.yPos,this.angleToPlayer(),18,1,8,0,1,5,"artillery"));
					}
					if(this.salvoDelay != 0){break;}
				}
			}
		}
	},
	...
	drawSelf: function(){
		...
		//Draw the box thing
		context.fillStyle = "rgb(255,255,255)";
		//this.temp = "rgb("+Math.round(Math.random()*255)+","+Math.round(Math.random()*255)+","+Math.round(Math.random()*255)+")";
...
var Plane = new Class({
	initialize: function(notTier,fromSide){
		...
		// Figure out if the plane should execute a loop, when, and in which direction.
		...
		// Enable depth charges when tier hits 9, and increae ammo counts for appropriate tiers.
	...
	handleWeapons: function(){
		...
		if(this.reloadCount <= 0){
			if(this.salvoCount == 0){
				...
				this.salvoCount--;
				this.reloadCount += this.salvoDelay;
				ordinance.push(new Bullet(this.xPos,this.yPos,this.angleToPlayer(),this.weaponSpeed,.5,2,0,1,5,"plane"));
			}
		}
	},
	angleToPlayer: function(){
	...
	updateSelf: function(){
		...
		if(player.lightningImpact == 0 && this.isCloseTo(player.xPos,player.yPos-player.yOffset/2,player.xSize/2,player.ySize/2)){
...
var MeteorStorm = new Class({
	...
	spawnStuff: function(){
		...
		if(Math.sin(this.heading) > 0){
			ordinance.push(new Bullet(0,fHeight*Math.random(),this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
		} else {
			ordinance.push(new Bullet(fWidth,fHeight*Math.random(),this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
		}
		
		if(Math.cos(this.heading) > 0){
			ordinance.push(new Bullet(fWidth*Math.random(),0,this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
		} else {
			ordinance.push(new Bullet(fWidth*Math.random(),fHeight,this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
		}
		/*
		for(i = 0; i < 1; i++){
			//Bullet.initialize: function(x,y,heading,speed,scale,damage,seaks,gravityMultiplier,inAccuracy,type){
			damageScale = Math.random();
			if(this.heading > Math.PI/2 && this.heading < 3*Math.PI/2){
				ordinance.push(new Bullet(fWidth*Math.random(),fHeight,this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
			} else {
				ordinance.push(new Bullet(fWidth*Math.random(),0,this.heading,20+10*damageScale,1+damageScale,5+5*damageScale,0,1,0,"meteor"));
			}
...
var Explosion = new Class({
	...
	drawSelf: function(){
		...
		if(this.shouldExist){
			...
			this.radialGradient = context.createRadialGradient(this.xPos,fHeight-this.yPos,0,this.xPos,fHeight-this.yPos,this.scale);
...
var OrbitalLaser = new Class({
	...
	killEverything: function(){
		...
		//ordinance.push(new Bullet(this.xPos-this.range,0,7*Math.PI/4,Math.random()*4+7,.8,5,0,0,90));
		//ordinance.push(new Bullet(this.xPos+this.range,0,Math.PI/4,Math.random()*4+7,.8,5,0,0,90));
...
var Bullet = new Class({
	...
	updateSelf: function(){
		if(this.shouldExist){
			...
			//Checks twice, once at current location, then at half way between current location and last location
			if(player.lightningImpact <= 0){
				if(this.isCloseTo(player.xPos,player.yPos-player.yOffset/2,player.xSize/2,player.ySize/2) || this.isCloseTo(player.xPos+this.xSpeed/2,player.yPos-player.yOffset/2+this.ySpeed/2,player.xSize/2,player.ySize/2)){
	...
	drawSelf: function(){		
		...
		//Draw some movement blur so it doesn't look like ass while moving quickly
...
var HealthCapsule = new Class({
	initialize: function(tier,fromSide){
		...
		this.shouldExist = true;
		this.hasProtection = true; //Remember kids, always use protection! You don't want to get the AIDS!
	},
	...
	spawnBullets: function(){
		for(var i = 0; i < 2; i++){
			...
			//initialize: function(x,y,heading,speed,scale,damage,seaks,gravityMultiplier,inAccuracy){
			
			ordinance.push(new Bullet(this.xPos,this.yPos,Math.PI,Math.random()*10+15,.2,-1,20,0,(180/Math.PI)*3));
		}
		//this.shouldSpawnBullets = false;
	},
...
var DepthCharge = new Class({
	...
	spawnBullets: function(){
		for(var i = 0; i < 15; i++){
			...
			ordinance.push(new Bullet(this.xPos,this.yPos,Math.PI*2,Math.random()*4+7,.8,5,0,0,(180/Math.PI)*6,"depth"));
		}
		//this.shouldSpawnBullets = false;
	},
	handleExisting: function(){
